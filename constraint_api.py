...
class Constraint:
    ...
    def __eq__(self, other):
        return (
            isinstance_Constraint(other)
            and self.var1 == other.var1
            and self.var2 == other.var2
            and self.constraint_fn.__code__.co_code
            == other.constraint_fn.__code__.co_code
        )
...
class ConstraintSatisfactionProblem:
    ...
    def get_domain(self, var):
        ...
        if var not in self.variables:
            raise KeyError(
                str(var) + " is not a variable in this problem." + str(self.variables)
            )
    ...
    def set_all_domains(self, domains_dict):
        ...
        if not set(domains_dict.keys()) <= set(self.variables):
            invalid_vars = [
                v for v in list(domains_dict.keys()) if v not in self.variables
            ]
    ...
    def set_assignment(self, var, val):
        ...
        if self.assignments.get(var) is not None:
            raise AttributeError(
                ...
                + " to value "
                + str(val)
                + ": var has already been assigned value "
                + str(self.assignments.get(var))
                + "."
            )
    ...
    def constraints_between(self, var1=None, var2=None):
        ...
        return [
            ...
            for e in [
                e
                if pred1(e.var1) and pred2(e.var2)
                else e.reverse()
                if pred2(e.var1) and pred1(e.var2)
                else None
    ...
    def set_unassigned_vars_order(self, unassigned_vars_ordered):
        ...
        unassigned vars."""
        if unassigned_vars_ordered is not None and not (
            set(unassigned_vars_ordered) <= set(self.variables)
        ):
            raise AttributeError(
                "unassigned_vars_ordered contains items that "
                + "are not variables in this problem"
            )
        if any(
            [var in list(self.assignments.keys()) for var in unassigned_vars_ordered]
        ):
            raise AttributeError(
                "unassigned_vars_ordered contains variables "
    ...
    def __str__(self):
        ...
        return (
            "ConstraintSatisfactionProblem with:"
    ...
    def __eq__(self, other):
        return (
            isinstance_ConstraintSatisfactionProblem(other)
