...
if python_version.major != 3:
    raise Exception(
        "Illegal version of Python for 6.034 lab. Detected Python "
        + "version is: "
        + str(sys.version)
    )
if python_version.minor == 5 and python_version.micro <= 1:
    raise Exception(
        "Illegal version of Python; versions 3.5.0 and 3.5.1 are disallowed "
...
constraint_dict = {
    "constraint_equal": constraint_equal,
...
def encode_constraint(constraint):
    ...
    if fn_name == "<lambda>":
        print(
            (
                " ** Note: Unfortunately, the online tester is unable to accept "
...
def encode_CSP(csp):
    return [
        csp.variables,
        list(map(encode_constraint, csp.constraints)),
        csp.unassigned_vars,
        csp.domains,
        csp.assignments,
    ]
...
def lambda_1(p, v):
    return len(p.get_domain(v)) == 1


def lambda_12(p, v):
    return len(p.get_domain(v)) in [1, 2]
...
def type_encode(arg):
    ...
    if isinstance_Constraint(arg):
        ...
        isinstance(arg, list)
        and len(arg) == 2  # special case for FUNCTION_WITH_CSP
    ...
    if mytype == "VALUE":
        ...
        for a in args:
            if isinstance_ConstraintSatisfactionProblem(a):
                return [attr(*args), a]
        raise Exception(
            "Test Error: 'FUNCTION_WITH_CSP' test missing CSP. "
            + "Please contact a TA if you see this error."
        )
    elif mytype == "MULTIFUNCTION":
        return [
            run_test((id, "FUNCTION", attr_name, FN), lab)
        ...
        return run_test(
            (id, "FUNCTION_WITH_CSP", attr_name, type_decode(args, lab)), lab
        )
    else:
        raise Exception(
...
def test_offline(verbosity=2):
    ...
    for index, (testname, getargs, testanswer, expected, fn_name, type) in enumerate(
        tests
    ):
        dispindex = index + 1
        ...
        try:
            ...
            print(
                "%d: (%s: No answer given, NotImplementedError raised)"
                % (dispindex, testname)
            )
            continue
...
def get_tarball_data(target_dir, filename):
    ...
    def add_dir(currentDir, t_verbose=True):
        for currentFile in os.listdir(currentDir):
            ...
            if tar_filter(currentFile):
                ...
                tar.add(
                    fullPath,
                    arcname=fullPath.replace(target_dir, top_folder_name, 1),
                    recursive=False,
                )
...
def test_online(verbosity=1):
    ...
    server.submit_code(
        username, password, lab.__name__, xmlrpc.client.Binary(tarball_data)
    )
    ...
    for index, testcode in enumerate(tests):
        ...
        correct, expected = server.send_answer(
            username, password, lab.__name__, testcode[0], type_encode(answer)
        )
...
def make_test_counter_decorator():
    ...
    def make_test(getargs, testanswer, expected_val, name=None, type="FUNCTION"):
        ...
        tests.append(
            (getargs_name, getargs, testanswer, expected_val, getargs_name, type)
        )
...
if __name__ == "__main__":
    if "submit" in sys.argv:
        ...
        if "IDLE" in sys.executable:
            ...
            print(
                "Local tests passed! Run 'python3 %s submit' to submit your code and have it graded."
                % sys.argv[0]
            )
